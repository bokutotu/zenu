...
/**
 * @brief Naive な 1D 畳み込み backward data 実装
 * @tparam T データ型 (float/double)
 * @param d_output 出力勾配テンソル (NCW: {N, K, L_out})
 * @param kernel カーネル (フィルタ) テンソル (KCS: {K, C, S})
 * @param d_input 入力勾配テンソル (NCW: {N, C, L})
 * @param input_shape 入力形状 {N, C, L}
 * @param kernel_shape カーネル形状 {K, C, S}
 * @param output_shape 出力形状 {N, K, L_out}
 * @param stride ストライド {stride}
            ...
            for (size_t l = 0; l < L_out; ++l) {
                ...
                for (size_t c = 0; c < C; ++c) {
                    for (size_t s = 0; s < S; ++s) {
                        int x = static_cast<int>(l * stride[0]) - static_cast<int>(padding[0]) + static_cast<int>(s * dilation[0]);
                        if (x >= 0 && x < static_cast<int>(L)) {
                            size_t in_idx = n * C * L + c * L + x;
                            size_t kernel_idx = k * C * S + c * S + s;
                            d_input[in_idx] += grad * kernel[kernel_idx];
                        }
                    }
                }
            }
...
/**
 * @brief CPU による 1D 畳み込み backward data テスト (float32)
 *
 * d_output は (N, K, L_out) = (2, 2, 28)、カーネルは (K, C, S) = (2, 3, 5)、
 * d_input は (N, C, L) = (2, 3, 28) の形状とします。<br>
 * ライブラリ実装による backward data の結果と naive 実装による結果が一致するか検証します。
 */
