...
void ZenuComputeConvCpuImpl::col2im1d(const void* col, void* input) const {
    const size_t N = this->input[0];
    const size_t C = this->input[1];
    const size_t W = this->input[2];
    ...
    const size_t out_w = this->output[2];

#define COL2IM_1D_LOOP(TYPE)                                         \
    do {                                                             \
        TYPE* in_ptr = static_cast<TYPE*>(input);                    \
        const TYPE* col_ptr = static_cast<const TYPE*>(col);         \
        /* 並列化対象を n と c に変更 */                              \
                ...
                for (size_t ow = 0; ow < out_w; ++ow) {                \
                    for (size_t kw = 0; kw < kernel_w; ++kw) {         \
                        ...
                        if (w_in >= 0 && w_in < static_cast<int>(W)) { \
                            const size_t col_index =                     \
                                ((n * out_w) + ow) * (C * kernel_w) + (c * kernel_w) + kw; \
                            const size_t in_index =                      \
                                (n * C + c) * W + static_cast<size_t>(w_in); \
                            in_ptr[in_index] += col_ptr[col_index];       \
                        }                                                \
                    }                                                    \
                }                                                        \
    ...
    switch (this->type) {
    case ZenuDataType::f32:
        COL2IM_1D_LOOP(float);
        break;
    case ZenuDataType::f64:
        COL2IM_1D_LOOP(double);
        break;
    default:
        std::cout << "Unsupported data type in col2im1d" << std::endl;
        break;
    }
#undef COL2IM_1D_LOOP
}
