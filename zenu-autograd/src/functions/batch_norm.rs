...
//         let var_mat = self.variance.get_data();
//         let mean_mat = self.mean.get_data();
//         let gamma_mat = self.gamma.get_data();
//         let beta_mat = self.beta.get_data();
//         let epsilon = self.epsilon.get_data();
//         let decay = self.decay.get_data();
//         let inv_std = self.inv_std.get_data_mut();
//         let output = self.output.upgrade().unwrap();
//         let output_mat = output.get_data_mut();
//
//         if is_train() {
//             D::batch_norm_2d_forward_train(
//                 input_mat, mean_mat, var_mat, gamma_mat, beta_mat, epsilon, decay, inv_std,
//                 output_mat,
//             );
//         }
//     }
...
//         let input = Matrix::<Owned<f64>, DimDyn, D>::from_vec(input, &[10, 3]);
//         let mean = Matrix::<Owned<f64>, DimDyn, D>::from_vec(vec![1., 2., 3.], &[3]);
//         let var = Matrix::<Owned<f64>, DimDyn, D>::from_vec(vec![1., 2., 3.], &[3]);
//         let decay = Matrix::<Owned<f64>, DimDyn, D>::from_vec(vec![0.9], &[]);
//         let epsilon = Matrix::<Owned<f64>, DimDyn, D>::from_vec(vec![1e-5], &[]);
//         let gamma = Matrix::<Owned<f64>, DimDyn, D>::from_vec(vec![1., 2., 3.], &[3]);
//         let delta = Matrix::<Owned<f64>, DimDyn, D>::from_vec(vec![1., 2., 3.], &[3]);
...
//             7.70009639,
//         ];
//         let ans = Matrix::<Owned<f64>, DimDyn, D>::from_vec(ans, &[10, 3]);
...
//             -1.2111522273820252e-17,
//         ];
//         let input_grad_ans = Matrix::<Owned<f64>, DimDyn, D>::from_vec(input_grad_ans, &[10, 3]);
//         let gamma_grad = vec![
//             2.220446049250313e-16,
//             2.220446049250313e-16,
//             2.220446049250313e-16,
//         ];
//         let gamma_grad = Matrix::<Owned<f64>, DimDyn, D>::from_vec(gamma_grad, &[3]);
//         let beta_grad = vec![10., 10., 10.];
//         let beta_grad = Matrix::<Owned<f64>, DimDyn, D>::from_vec(beta_grad, &[3]);
...
//
//     // #[test]
//     fn batch_norom_meduium_4d<D: Device>() {
//         let input = (1..(10 * 3 * 3 * 3 + 1))
//             .map(|x| x as f64)
//             .collect::<Vec<f64>>();
//         let input = Matrix::<Owned<f64>, DimDyn, D>::from_vec(input, &[10, 3, 3, 3]);
//         let mean = Matrix::<Owned<f64>, DimDyn, D>::from_vec(vec![1., 2., 3.], &[3]);
//         let var = Matrix::<Owned<f64>, DimDyn, D>::from_vec(vec![1., 2., 3.], &[3]);
//         let decay = Matrix::<Owned<f64>, DimDyn, D>::from_vec(vec![0.9], &[]);
//         let epsilon = Matrix::<Owned<f64>, DimDyn, D>::from_vec(vec![1e-5], &[]);
...
//             7.852143897252609,
//         ];
//         let ans = Matrix::<Owned<f64>, DimDyn, D>::from_vec(ans, &[10, 3, 3, 3]);
...
//         ];
//         let input_grad_ans =
//             Matrix::<Owned<f64>, DimDyn, D>::from_vec(input_grad_ans, &[10, 3, 3, 3]);
