    ...
    F: Fn(Variable<T, D>) -> Variable<T, D> + Copy,
{
    ...
    if bidirectional {
        ...
        // Stack the outputs into a Variable of shape [seq_len, batch_size, num_directions * hidden_size]
        let output = stack(&outputs, 0);

        output
    } else {
        // Stack the forward outputs into a Variable
        let output = stack(&forward_output, 0);
        output
    }
}
    ...
    F: Fn(Variable<T, D>) -> Variable<T, D> + Copy,
{
    ...
    for layer in 0..num_layers {
        let layer_weight = &weights[layer];
...
//         functions::rnn::naive::{rnn_relu, RNNLayerWeights, RNNWeights},
//         Variable,
//     };
//
//     fn rnn_test_single_layer<D: Device>(path: &str, bidirectional: bool) {
...
//         };
//
//         let weight_backward = if bidirectional {
//             Some(RNNWeights {
//                 weight_input: input_weight.clone(),
//                 weight_hidden: hidden_weight.clone(),
//                 bias_input: input_bias.clone(),
//                 bias_hidden: hidden_bias.clone(),
//             })
//         } else {
//             None
//         };
...
//         let num_directions = if bidirectional { 2 } else { 1 };
//
//         let h = zeros([1 * num_directions, batch_size, hidden_size]);
//
//         let output = rnn_relu(input.clone(), h, &weights, bidirectional);
//         output.backward();
...
//         rnn_test_single_layer::<D>("../test_data_json/rnn_fwd_bkwd_single.json", false);
//     }
//     run_test!(rnn_single_layer, rnn_single_layer_cpu, rnn_single_layer_gpu);
//
//     fn rnn_single_layer_bidirectional<D: Device>() {
//         rnn_test_single_layer::<D>(
//             "../test_data_json/rnn_fwd_bkwd_single_bidirectional.json",
//             true,
//         );
//     }
//     run_test!(
//         rnn_single_layer_bidirectional,
//         rnn_single_layer_bidirectional_cpu,
//         rnn_single_layer_bidirectional_gpu
//     );
//
//     fn rnn_single_layer_seq_1<D: Device>() {
//         rnn_test_single_layer::<D>(
//             "../test_data_json/rnn_fwd_bkwd_single_seq_len_1.json",
//             false,
//         );
//     }
//     run_test!(
