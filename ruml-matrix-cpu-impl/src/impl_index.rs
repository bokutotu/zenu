...
macro_rules! impl_owned_slice {
    ($dim:ty, $index:ty) => {
        impl<'a, O: Memory> MatrixSlice<'a, $index> for CpuMatrix<O, $dim> {
            type Output = impl ViewMatrix<'a>;

            fn slice(&'a self, index: $index) -> Self::Output {
                ...
                CpuMatrix::new(data, new_shape_stride.shape(), new_shape_stride.stride())
            }
        }
    };
}
impl_owned_slice!(Dim1, Slice1D);
impl_owned_slice!(Dim2, Slice2D);
impl_owned_slice!(Dim3, Slice3D);
impl_owned_slice!(Dim4, Slice4D);
...
// #[test]
// fn test_index() {
//     let owned = CpuMatrix::from_vec(vec![1., 2., 3., 4.], Dim1::new([4]));
//
